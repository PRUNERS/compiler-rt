...
#if !defined(TSAN_NO_LOCAL_CONCURRENCY)
void ReleaseForceStore(ThreadState *thr, uptr pc, uptr addr) {
  DPrintf("#%d: ReleaseForceStore %zx\n", thr->tid, addr);
  if (thr->ignore_sync)
    return;
  SyncVar *s = ctx->metamap.GetOrCreateAndLock(thr, pc, addr, true);
  thr->fast_state.IncrementEpoch();
  ...
  ReleaseForceStoreImpl(thr, pc, &s->clock);
  s->mtx.Unlock();
}
...
#if !defined(TSAN_NO_LOCAL_CONCURRENCY)
void ReleaseForceStoreImpl(ThreadState *thr, uptr pc, SyncClock *c) {
  if (thr->ignore_sync)
    return;
  thr->clock.set(thr->fast_state.epoch());
  thr->fast_synch_epoch = thr->fast_state.epoch();
  thr->clock.ReleaseForceStore(&thr->proc()->clock_cache, c);
